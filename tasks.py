...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_BGP_MODELS_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_bgp_models.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    if (
        context.nautobot_bgp_models.ephemeral_ports
        and context.nautobot_bgp_models.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_bgp_models.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(os.path.join(context.nautobot_bgp_models.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_bgp_models.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_bgp_models.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_bgp_models.python_ver
    dockerfile_path = os.path.join(context.nautobot_bgp_models.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_bgp_models.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_bgp_models'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_bgp_models")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_bgp_models.migrations"
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_bgp_models/templates/"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for BGP Models."""
